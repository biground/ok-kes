...
```bash
pyinstaller --onefile --noconsole --uac-admin --noupx --runtime-tmpdir "C:\Temp\ok_kes" --name "卡厄思自动化工具v1.3.0" --icon icons/icon.ico ^
...
```bash
rmdir /s /q build
del "卡厄思自动化工具v1.3.0.spec"
```

### 4. 产物

位于 `dist\卡厄思自动化工具v1.3.0.exe`，约 265 MB。
