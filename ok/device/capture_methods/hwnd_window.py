    ...
    def try_resize_to(self, resize_to):
        ...
        if self.hwnd and self.window_width > 0:
            ...
            if resize_width > 0:
                ...
                # 屏幕太小放不下任何 resize_to 目标（如 1080p 屏幕 1920×1080 < 1920+边框），
                # 将窗口填满整个屏幕，使客户区最大化
                fill_width = screen_width
                fill_height = screen_height
                logger.info(f'屏幕太小，填充窗口至 {fill_width}x{fill_height}')
                resize_window(self.hwnd, fill_width, fill_height)
                self.do_update_window_size()
                if self.window_width == fill_width and self.window_height == fill_height:
                    logger.info(f'填充窗口成功，客户区 {self.width}x{self.height}')
                    return True
                else:
                    logger.error(f'填充窗口失败: {self.width}x{self.height}')
